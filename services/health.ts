...
export async function getPregnancyCommuteAdvice(topic: string): Promise<string> {
...
export async function getPregnancyMedicalInfo(query: string): Promise<string> {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const response = await ai.models.generateContent({
...
export async function getGeneralPregnancyInfo(query: string): Promise<string> {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const response = await ai.models.generateContent({
            model: "gemini-2.5-pro",
